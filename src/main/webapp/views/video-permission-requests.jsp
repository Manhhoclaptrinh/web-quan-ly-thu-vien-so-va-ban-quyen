...
<c:choose><c:when test="${sessionScope.currentUser.role == 'ADMIN' or sessionScope.currentUser.role == 'LIBRARIAN'}"><p class="text-muted">Duyệt hoặc từ chối yêu cầu của READER.</p></c:when><c:otherwise><p class="text-muted">Gửi yêu cầu xem/tải video và theo dõi trạng thái xử lý.</p>
