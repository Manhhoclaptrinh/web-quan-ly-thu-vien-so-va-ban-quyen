...
<head>
    <meta charset="UTF-8">
    <title>Quản lý bản quyền video - Thư viện số</title>
...
<div class="container">
    <div class="card">
        ...
        <form action="${pageContext.request.contextPath}/video-license/save" method="post"><c:if test="${not empty editLicense}"><input type="hidden" name="licenseId" value="${editLicense.licenseId}"></c:if>
            <div class="form-group">
                ...
                <select name="videoId" class="form-control" required>
                    <c:forEach var="v" items="${videos}">
                        <option value="${v.videoId}" ${not empty editLicense and editLicense.videoId == v.videoId ? 'selected' : ''}>${v.title}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="licenseCode" class="form-control" placeholder="VD: VLIC-0001" value="${editLicense.licenseCode}">
            </div>
            <div class="form-group">
    ...
    <div class="card">
        <h2>Danh sách bản quyền video</h2>
        <c:choose>
            <c:when test="${empty licenses}">
                <p class="text-muted">Chưa có bản quyền video nào được cấp.</p>
            </c:when>
            <c:otherwise>
                <table>
                    <thead>
                    ...
                    <c:forEach var="lic" items="${licenses}">
                        <tr>
                            ...
                            <td>
                                <a href="${pageContext.request.contextPath}/video-license/edit?id=${lic.licenseId}">Sửa</a> | <a href="${pageContext.request.contextPath}/video-license/delete?id=${lic.licenseId}"
                                   onclick="return confirm('Xóa bản quyền video này?');" style="color:#dc2626;">Xóa</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
